...
class AskForHelpController extends Controller
{
    public function createOrder(Request $request){
        ...
        if (!$title||!$content||!$category||!$closeTime||!$money){
            return Response::json(['statue' => 400,'msg' => 'need title or content or category or close_time or money']);
        }
        if (!Order::create(['title' => $title,'content' => $content,'category' => $category,'state' => 0,'closeTime' => $closeTime,'money' => $money,'applicant' => $phone])){
