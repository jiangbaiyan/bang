...
class CreateOrdersTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('orders', function (Blueprint $table) {
            $table->increments('id');
            $table->string('uuid')->nullable()->comment('唯一订单号');
            ...
            $table->tinyInteger('status')->default(0)->nullable()->comment('0-未发布 1-已发布 2-正在进行 3-服务完成 4-评价完成 5-订单取消');
            $table->unsignedDecimal('price',8,2)->nullable()->comment('酬金');
            $table->unsignedDecimal('longitude',9,6)->nullable()->comment('经度');
            $table->unsignedDecimal('latitude',9,6)->nullable()->comment('纬度');
            $table->softDeletes();
